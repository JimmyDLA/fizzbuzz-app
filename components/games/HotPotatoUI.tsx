...
export function HotPotatoUI() {
  ...
  return (
    <View className="flex-1 items-center justify-center pt-2 w-full px-2">
      {amIHolder ? (
        <Text className="text-red-400 text-6xl font-black mb-8 text-center uppercase tracking-widest" style={{ textShadowColor: 'rgba(255,0,0,0.5)', textShadowOffset: { width: 3, height: 3 }, textShadowRadius: 5 }}>
      ...
      <View className="items-center justify-center h-80 w-full mb-8 relative">
        {amIHolder ? (
          <TouchableOpacity activeOpacity={0.6} onPress={handlePass} disabled={timer <= 0}>
            <Animated.View style={timer <= 0 ? {} : { transform: [{ scale: scaleAnim }] }} className={`w-64 h-80 bg-orange-800 rounded-[100px] border-[16px] border-orange-900 items-center justify-center ${timer <= 0 ? 'opacity-50' : ''}`}>
