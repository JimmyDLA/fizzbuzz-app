...
export default function GameScreen() {
  ...
  const { playerName, players: reduxPlayers, gamePhase, timer, currentGameType, currentCategory, lastWinners, lastLosers } = useSelector((state: any) => state.lobby);
  ...
  const renderResolution = () => {
    ...
    return (
      <View className="flex-1 bg-purple-600 justify-center items-center px-6 pt-12">
        <Text className="text-white text-6xl font-black mb-10 text-center shadow-lg uppercase" style={{ textShadowColor: 'rgba(0,0,0,0.3)', textShadowOffset: { width: 3, height: 3 }, textShadowRadius: 1 }}>RESULTS!</Text>
        ...
        <View className="w-full bg-black/30 rounded-[40px] p-6 mb-12 border-4 border-white/20">
          <View className="flex-row items-center justify-center mb-4">
            ...
            <Text className="text-yellow-400 font-black text-2xl text-center tracking-widest uppercase ml-1">)</Text>
          </View>
          ...
          <Text className="text-blue-300 font-black text-2xl mb-4 text-center tracking-widest uppercase mt-2">LOSERS (+1 Drink)</Text>
